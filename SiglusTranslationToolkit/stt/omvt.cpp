...
static void setframe(STREAMSTAT *s)
{
	...
	f->start = (int)round((float)s->frame * s->frametime);
	f->end = (int)round(f->start + s->frametime);
...
static void omv_setup(STREAMSTAT *s)
{
	...
	h->frametime = (int)((float)ti.fps_denominator / (float)ti.fps_numerator * 1000000.0f);
...
namespace stt
{
	struct OMVHeader
	{
		unsigned int DataOffset; //ogv(1.0) or package index(1.1)
		unsigned char  MainVersion;
		unsigned char  SubVersion;
		unsigned short  Unknown1;
		unsigned int UnkArray[8];
		unsigned int EncryptType; //1--32bit(Alpha) 2--24bit(Non Alpha)
		unsigned int Width;
		unsigned int Height;
		unsigned int UnkArray2[2];
		unsigned int FrameTime;
		ULARGE_INTEGER StreamID;
		int  Flag2;
		unsigned int DataPackageCount;
		unsigned int FrameCount;

		OMVHeader() {}

		OMVHeader(std::vector<unsigned char>& raw)
		{
			std::copy(raw.begin(), raw.begin() + sizeof(OMVHeader), reinterpret_cast<char*>(this));
		}
	};
	...
	struct DataPackage
	{
		...
		unsigned int Unk;
		unsigned int FrameCount;
		unsigned int IndexOfFirstFrame;
	};
	...
	struct FramePackage
	{
		...
		unsigned int PackageId;
		unsigned int IndexOfThisPackage;
		unsigned int FrameTime;
		unsigned int Unk[2];
	...
	{
		std::wstring outFileOGV = outPath + getPartialFileName(inFile) + L".ogv";
		...
		unsigned int oggsTag = 0;
		std::copy(buf.begin() + offset, buf.begin() + offset + sizeof(oggsTag), reinterpret_cast<char*>(&oggsTag));
		...
		std::wcout << getPartialFileName(inFile) << L" Converting..." << std::endl;

		if (omvHeader.EncryptType == 1)
		{
			throw(ERR_NO_SUPPORT3);
		}

		buf = std::vector<unsigned char>(buf.begin() + offset, buf.end());

		std::ofstream ofStream;
		checkAvailablePath(outFileOGV, true);
		ofStream.open(outFileOGV, std::ios::binary);
		ofStream.write(reinterpret_cast<char*>(&buf[0]), buf.size());
		ofStream.close();
	}

	void RepackOMV(std::wstring inFile, std::wstring outPath)
	{
		std::wstring outFileOMV = outPath + getPartialFileName(inFile) + L".omv";

		int len = WideCharToMultiByte(CP_ACP, 0, &inFile[0], -1, nullptr, 0, nullptr, nullptr);
		std::string inFileA(len, 0);
		WideCharToMultiByte(CP_ACP, 0, &inFile[0], -1, &inFileA[0], len, nullptr, nullptr);

		len = WideCharToMultiByte(CP_ACP, 0, &outFileOMV[0], -1, nullptr, 0, nullptr, nullptr);
		std::string outFileOMVA(len, 0);
		WideCharToMultiByte(CP_ACP, 0, &outFileOMV[0], -1, &outFileOMVA[0], len, nullptr, nullptr);


		pack_omv(inFileA.c_str(), outFileOMVA.c_str());
	}
}
//
